...
> **Update (2026-08-27): same-URL republish (AC-10) is dropped.** The published vCard URL is stored in each user's Office 365 / Exchange mailbox attribute (an "Other Attribute") as the source of truth, and the QR code / email signature is generated from that attribute. So a changing URL on update is fine: updating a card is a fresh upload that yields a new URL, and the operator updates the attribute. This removes the need for the address to survive an update. What replaces it: publishing keeps a durable **private** copy (so a card can be unpublished and its object retired), and **unpublish**/**delete** retire the old public URL. Slugs are still retired-not-recycled. A future Microsoft Graph integration may write the published URL into the Exchange attribute automatically.
