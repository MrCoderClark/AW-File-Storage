...
| [0016-o365-directory-auto-cards.md](0016-o365-directory-auto-cards.md) | Per-org opt-in: auto-create + publish a contact vCard from each licensed Office 365 user's directory details and write its URL into CustomAttribute1, near-real-time by polling (mailbox-readiness gated) + nightly; offboard on disable/unlicense. No app logins (not SCIM) | Extends the O365 sync (0010/0013) from writing URLs for human-made cards to creating the cards from the directory |
| [0017-o365-offboarding-card-removal.md](0017-o365-offboarding-card-removal.md) | Per-org opt-in: when an O365 user is offboarded (disabled AND unlicensed, positive signal only), retract their published card — auto or manual, on the org's own O365 domain — clear CustomAttribute1, and hard-delete after a 30-day grace period | Closes the lifecycle opened by 0016 — the departure end, covering human-made cards too |
| [0018-activity-logs.md](0018-activity-logs.md) | A read-only Activity Logs page (member-accessible) over the audit trail: filterable/searchable/paginated, with human categories, statuses, actors, and resolved target names (no UUIDs); Eastern timestamps; responsive table/cards; reached from the side rail, replacing Recent Activity | Surfaces the spec-0002 audit log — especially the 0016/0017 onboarding/offboarding events — as a usable UI |
| [0019-o365-hard-delete-offboarding.md](0019-o365-hard-delete-offboarding.md) | Catch users fully DELETED from O365 (not just disabled): tolerate the 404 when clearing a gone user's attribute (stops a recurring sync error), and detect deletions from Entra's recycle bin (positive signal, matched by stored user id) to retract their card on the same terms as spec 0017 | Closes the hard-delete blind spot in spec 0017's offboarding |
| [0020-http-security-headers.md](0020-http-security-headers.md) | Add a single, architecture-matched set of HTTP security headers to every response (CSP incl. `frame-ancestors 'none'`, HSTS, `nosniff`, `Referrer-Policy`, `Permissions-Policy`), a script-free CSP on the public card/logo responses, and close a latent single-quote escaping gap — proven report-only before enforce so hydration never breaks | Security-audit hardening (SECURITY.md) of the `Auth`/`UI`/delivery rows: clickjacking, transport, sniffing, and XSS defense-in-depth |
| [0021-role-assignment-authorization.md](0021-role-assignment-authorization.md) | Close a privilege-escalation: an admin can currently grant/assume the `owner` role and act on owners. Restrict granting `owner`, and any change to an owner account, to owners — enforced in the `members.ts` service, not the UI | Security-audit hardening of the tenancy/role model (broken access control, OWASP A01) |
| [0022-abuse-resistance-rate-limiting.md](0022-abuse-resistance-rate-limiting.md) | Add layered abuse resistance: Cloudflare WAF rate-limit rules for the public card + auth surfaces, cache/bound the expensive public generators (QR, PDF), a thin DB-backed app throttle on sensitive authenticated actions, and vCard field bounds | Security-audit hardening of availability/cost (rate limiting + resource exhaustion) |
| [0023-better-auth-upgrade.md](0023-better-auth-upgrade.md) | Deferred: upgrade Better Auth off the pinned 1.4.21 to a patched release (with `@better-auth/cli` in lockstep) to clear a standing critical advisory bundle. No currently reachable exploit (the CVEs are for unused OAuth/OIDC/magic-link/admin/SCIM flows; the one organization-plugin advisory is blocked by `disableSignUp` + email verification), but done properly: inspect the migration for a D1 parent-table rebuild (gotcha #9), retest every auth flow | Dependency-security hardening of the `Auth` row (npm audit follow-up) |
| [0024-help-documentation-cms.md](0024-help-documentation-cms.md) | Replace the placeholder header bell with a `?` help icon opening a slide-over Help drawer, backed by an admin-editable help CMS: per-org rich-text articles (Tiptap, draft/publish, image uploads), platform-owner sharing across orgs, a `/help` library page, contextual + client-side search, and a contact-support link. Content in D1, images in R2, admin HTML sanitized against stored XSS | Extends the `UI` row with in-app documentation; a new content surface for staff |
| [0025-help-knowledge-base.md](0025-help-knowledge-base.md) | Extends 0024 into a full knowledge base matching the design mocks: move authoring out of Settings into a dedicated admin **Knowledge base** CMS shell (admin/owner only), add managed nestable **categories**, **tags**, a **featured image**, **related articles**, and a per-article **audience** (all staff vs admins-only), and upgrade the published article page to the mock. Reader drawer/`/help` stay; queries gain the audience filter. All additive on 0024's per-org + platform-owner-shared, isolation-tested model | Grows the in-app documentation (0024) into a knowledge base; extends the `UI` row |
