...
| [0015-scim-provisioning.md](0015-scim-provisioning.md) | The app as a SCIM 2.0 server: a customer's Entra pushes user create/update/deactivate over a per-org bearer token (hashed, one-org scope, least-privilege member-only), automating onboarding and offboarding; set-password email deferred ~5 min for mailbox readiness | Automates the membership layer of spec 0014 from the customer's identity provider |
