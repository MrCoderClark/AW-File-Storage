...
- **Branch:** `phase-7-files-view` (Phase 7 built, **not yet committed**). Phase 6 is merged + deployed to prod (with the fixes below); **Phase 7 is undeployed**.
...
1. **Phase 7 — commit, PR/merge, then deploy.** Uncommitted on `phase-7-files-view`. `npm run typecheck` + `npm test`, commit/PR/merge, then `npm run deploy`. No migration involved.
