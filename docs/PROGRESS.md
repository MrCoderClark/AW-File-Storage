...
- **Branch:** `phase-8-2fa` (Phase 8 built, **not yet committed**). Phase 7 is merged; deploy Phase 7 + 8 together when ready.
...
1. **Phase 8 — commit, PR/merge, then deploy.** Uncommitted on `phase-8-2fa`. First `npm install qrcode && npm install -D @types/qrcode`, run the `member.twoFactorRequired` regen + migration (⚠️ inspect the SQL — must be a `member` ALTER, not an `organization`/`user` rebuild; gotcha #9), then `npm run typecheck` + `npm test`, commit/PR/merge, then `npm run deploy` (this migration DOES need `--remote` before deploy). The deferred **sign-in 2FA step is now done** (was item 4).
