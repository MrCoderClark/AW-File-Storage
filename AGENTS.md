...
Status: **DEPLOYED to production; Phases 0–4b done, Phase 4c next.** The app is live on Cloudflare Workers (Paid plan) at `https://aw-file-storage.aw-file-storage.workers.dev` with a working owner account, and the full loop works (browser upload → publish vCard → live public URL). Built: scaffold; tenancy + org isolation (0002); Better Auth — sessions, org scoping, CSRF, lockout, breach check, email, invitations, 2FA (0001); full upload/publish pipeline (0003); and the app shell + interactive Upload Center (0004 phases 4a/4b). **See [docs/PROGRESS.md](docs/PROGRESS.md) for the full handoff** — current state, production config, known issues, and the ordered TODO (4c file list + rail data, cron-worker deploy, deferred auth-UI pages, custom domain). Public vCards currently serve from the bucket's `r2.dev` URL (`PUBLIC_FILE_DOMAIN`).
