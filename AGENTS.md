...
Status: **DEPLOYED to production; built through Phase 22 (specs 0001–0019).** Live at `https://www.awvcard.com` (custom domain). The full loop works and then some: browser upload / Create-Card wizard → publish vCard → **styled public landing page** + `.vcf` at `https://contacts.awvcard.com/c/<slug>` (served by the app Worker since Phase 11) → **view/scan/download analytics** (spec 0008) → optional **Office 365 CustomAttribute1 sync** via Microsoft Graph (spec 0010, toggle-gated). Also built: tenancy + org isolation (0002); Better Auth sessions/CSRF/lockout/breach-check/email/invitations/2FA (0001); upload/publish pipeline (0003); app shell + Upload Center + dashboard (0004); user management (0005); Create-Card wizard + edit-in-place (0006); Files view with server pagination/search (0007); printable signature + QR + host-based card access + Settings toggles (0009); O365 certificate auth (0011); **multi-tenant isolation hardening + org lifecycle** (0012); **per-org Office 365 credentials, encrypted at rest** (0013); **domain-based user provisioning console** (0014). **SCIM (0015) was built, validated, then removed** as the wrong approach — replaced by the **full O365 contact-card lifecycle**: **auto-create cards from the O365 directory** (0016, per-org opt-in, forward-only); **offboarding removal** — retract + 30-day-delete a departed user's card, covering manual cards too (0017); a read-only **Activity Logs** page over the audit trail (0018, member-accessible, in the side rail); and **hard-delete offboarding** — catch soft- *and* permanently-deleted users, delete permanently-deleted cards immediately, and a 404-tolerant attribute clear (0019). **See [docs/PROGRESS.md](docs/PROGRESS.md) for the full handoff** — current state, production config, the migration-before-deploy lesson, local-dev gotchas, and the ordered TODO.
...
| Auth | Better Auth **1.7.3** (self hosted, spec 0023), D1 Drizzle adapter via `@better-auth/drizzle-adapter`, `organization` + `twoFactor` + `haveIBeenPwned` + `nextCookies` plugins — **not** hand written, **not** a hosted IdP |
...
2. **One way to know the caller.** `getSession()`, `requireOrgRole(role)`, and `requireApiRole(role)` (in `src/server/session.ts`) are the only ways feature code learns who the caller is or gates on a role. Nothing else parses the cookie or reads `member.role`. Owner is a strictly higher tier than admin: only an owner may grant the owner role or act on an owner account (role, status, removal, password, reset link, sessions, 2FA). See specs 0001 and 0021.
...
- **Security headers and an enforced Content-Security-Policy live in `next.config.ts` `headers()`** (spec 0020). The CSP is enforced, so a resource from any new outside origin (a script, style, font, image, or fetch target) is blocked until you add that origin to `APP_CSP` there. The public `/c/*` route sets its own stricter CSP in the route handler that allows no scripts at all. Framing is blocked by `X-Frame-Options: DENY` and the CSP. Cloudflare edge rate limits and the QR cache rule are documented in `config/cloudflare-rate-limits.md`.
- **Better Auth is on 1.7.3** (upgraded from 1.4.21, spec 0023). The old standalone `@better-auth/cli` was deprecated and frozen at 1.4.21 — that freeze is why the pin existed. It is replaced by the `auth` package, versioned in lockstep with the runtime, so there is no pin any more; bump `better-auth`, `@better-auth/drizzle-adapter`, and the `auth` CLI together. 1.7 also adds the built-in 2FA-code lockout (`two_factor.verified` / `failed_verification_count` / `locked_until`).
- After changing Better Auth plugins/options, regenerate the schema with `npx auth@latest generate --config ./better-auth.config.ts --output ./src/server/db/auth-schema.ts --adapter drizzle --dialect sqlite -y`, then `drizzle-kit generate` — **INSPECT the SQL for a parent-table rebuild before applying (gotcha #9)** — then `wrangler d1 migrations apply`.
